...
class Navigator {
  ...
  func show(destination: Destination, sender: UIViewController) {
    ...
    case .login:
      ...
      show(target: LoginViewController.createWith(viewModel: vm),
           sender: sender)
    case .repoList:
    ...
    case .issueList(let repoId):
      ...
      show(target: IssueListViewController.createWith(viewModel: vm),
           sender: sender)
    case .setting:
      let vm = SettingViewViewModel()
      show(target: SettingViewController.createWith(viewModel: vm),
           sender: sender)
    case .createIssue(let repoId):
      let vm = CreateIssueViewViewModel(repoId: repoId)
      show(target: CreateIssueViewController.createWith(viewModel: vm),
           sender: sender)
    case .issueDetail(let repoId, let issueId):
      let vm = IssueDetailViewViewModel(repoId: repoId, issueId: issueId)
      show(target: IssueDetailViewController.createWith(viewModel: vm),
           sender: sender)
    }
  }
